...
import { SortColumn, SortDirection } from "../../directive/orderdata.directive";
...
function matches(details: ORDERS, term: string, _pipe: PipeTransform) {
  return (
    details.product.toLowerCase().includes(term.toLowerCase()) ||
    details.status.toLowerCase().includes(term.toLowerCase()) ||
    details.color.toLowerCase().includes(term.toLowerCase()) ||
